...
const config: Config = {
  nationToken:
    ...
    defaultConfig.nationToken ||
    zeroAddress,
  veNationToken:
    process.env.NEXT_PUBLIC_VENATION_ADDRESS ||
    defaultConfig.veNationToken ||
    zeroAddress,
  veNationRequiredStake:
    Number(process.env.NEXT_PUBLIC_VENATION_REQUIRED_STAKE) || 2,
  veNationRewardsMultiplier:
    Number(process.env.NEXT_PUBLIC_VENATION_REWARDS_MULTIPLIER) || 2.5,
  balancerVault:
    process.env.NEXT_PUBLIC_BALANCER_VAULT_ADDRESS ||
    defaultConfig.balancerVault ||
    zeroAddress,
  balancerPoolId:
    process.env.NEXT_PUBLIC_BALANCER_NATION_ETH_POOL_ID ||
    defaultConfig.balancerPoolId ||
    zeroAddress,
  balancerLPToken:
    process.env.NEXT_PUBLIC_BALANCER_NATION_ETH_POOL_TOKEN ||
    defaultConfig.balancerLPToken ||
    zeroAddress,
  lpRewardsContract:
    process.env.NEXT_PUBLIC_LP_REWARDS_CONTRACT_ADDRESS ||
    defaultConfig.lpRewardsContract ||
    zeroAddress,
  nationPassportNFT:
    process.env.NEXT_PUBLIC_PASSPORT_NFT_ADDRESS ||
    defaultConfig.nationPassportNFT ||
    zeroAddress,
  nationPassportNFTIssuer:
    process.env.NEXT_PUBLIC_PASSPORT_NFT_ISSUER_ADDRESS ||
    defaultConfig.nationPassportNFTIssuer ||
    zeroAddress,
  nationDropContracts: JSON.parse(
    process.env.NEXT_PUBLIC_NATION_DISTRIBUTOR_CONTRACT_ADDRESS || 'null'
  ) ||
    defaultConfig.nationDropContracts || [zeroAddress],
