...
test("returns undefined if no lock amount", () => {
  const date = getPassportExpirationDate(BigNumber.from(0), BigNumber.from(0));
...
test("returns past date if below threshold", () => {
  const date = getPassportExpirationDate(BigNumber.from(1).mul(String(10 ** 18)), BigNumber.from(inFourYears));
  expect(date).not.toBe(undefined);
  expect(date!.getTime() < Date.now()).toBe(true);
})

test("returns future date if over threshold", () => {
  const date = getPassportExpirationDate(BigNumber.from(2).mul(String(10 ** 18)), BigNumber.from(inFourYears));
  expect(date).not.toBe(undefined);
  expect(date!.getTime() > Date.now()).toBe(true);
})
